...
print_usage() {
        ...
        echo "          -n  Number of images extracted from each video (200 by default)"
...
function dl_vids() {
    ...
    $downloader -f "bestvideo[ext=mp4]+bestaudio[ext=m4a]/mp4"  -o "$tmp_directory/%(title)s.mp4" "$video_url" &> /dev/null

    if [ $? -eq 0 ]; then
        colored_echo "1;32m" "Download completed successfully."
        log_verbose "Download details: Video URL - $video_url"
    else
        colored_echo "1;31m" "Error: Download failed."
        exit 1
    fi
}
...
function extract_images() {
    ...
    for file in "$tmp_directory"/*.mp4; do
        ...
        log_verbose "Extracting images from $file with interval $frame_interval..."
        ffmpeg -i "$file" -vf "select=not(mod(n\,$frame_interval))" -vsync vfr "$output_directory/${filename}_%04d.png" &> /dev/null
